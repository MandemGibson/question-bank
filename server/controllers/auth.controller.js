...
async function loginHandler(req, res, next) {
    try {
        ...
        if (!isEqual) return res.status(400).json({ message: 'Invalid userId or password' })
        ...
        const { id } = await getSessionByUserId({ userId, ip: req.ip }) ?? await createSession({ userId, ip: req.ip })
